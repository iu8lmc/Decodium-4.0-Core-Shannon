...
private slots:
    ...
    // ----------------------------------------------------------------
    // ATS: with fewer than kAtsMinSamples (3) partner arrivals tracked,
    // the engine must NOT delay TX emission. lastAtsOffsetMs stays 0
    // and the atsOffsetApplied signal reports samples<3 with offset=0.
    // ----------------------------------------------------------------
    void atsNoOpWhenConfidenceMissing() {
        auto cfg = makeCfg();
        cfg.adaptiveTxSyncEnabled = true;
        Ft2QsoEngine eng(cfg, nullptr);
        TimePoint synthetic = Clock::now();
        eng.setClockOverride([&]() { return synthetic; });

        QSignalSpy atsSpy(&eng, &Ft2QsoEngine::atsOffsetApplied);
        eng.enableTx(true); // CallingCq -> requestTx(6) (no partner yet)
        QCOMPARE(eng.atsSampleCount(), std::size_t(0));
        QVERIFY(!eng.atsHasConfidence());
        QCOMPARE(eng.lastAtsOffsetMs(), 0);

        // Engage K1ABC on grid response — first partner arrival is registered
        // AFTER requestTx, so this requestTx still saw 0 samples.
        DecodeRow r0 = makeRow("K1ABC", "IK8TWX K1ABC FN42");
        r0.receivedAt = synthetic;
        eng.feedParsed({r0});
        QVERIFY(std::holds_alternative<state::AwaitingReport>(eng.state()));
        QCOMPARE(eng.atsSampleCount(), std::size_t(1));
        QVERIFY(!eng.atsHasConfidence());
        QCOMPARE(eng.lastAtsOffsetMs(), 0);

        // Every atsOffsetApplied so far must have offset 0 (no confidence).
        QVERIFY(atsSpy.count() >= 1);
        for (int i = 0; i < atsSpy.count(); ++i) {
            QCOMPARE(atsSpy.at(i).at(0).toInt(), 0);
        }
    }

    // ----------------------------------------------------------------
    // ATS: once 3+ partner samples are accumulated, requestTx delays
    // the next TX emission to land in the partner's predicted RX window.
    // The applied offset must be positive and within ±kAtsMaxCorrection.
    // ----------------------------------------------------------------
    void atsAppliesPartnerOffsetAfterConfidence() {
        auto cfg = makeCfg();
        cfg.adaptiveTxSyncEnabled = true;
        Ft2QsoEngine eng(cfg, nullptr);
        TimePoint synthetic = Clock::now();
        ...
        eng.enableTx(true);
        DecodeRow r0 = makeRow("K1ABC", "IK8TWX K1ABC FN42", -10, 1500);
        r0.receivedAt = synthetic;
        eng.feedParsed({r0}); // sample 1; AwaitingReport
        QVERIFY(std::holds_alternative<state::AwaitingReport>(eng.state()));

        // Seed 4 more partner arrivals via bare reports (re-emit TX2 is guard-
        // skipped since msg is unchanged, but the post-visit ATS sampling
        // still pushes the timestamp into the tracker).
        for (int i = 0; i < 4; ++i) {
            synthetic += std::chrono::milliseconds(7500); // 2 slot periods
            DecodeRow ri = makeRow("K1ABC",
                                   QString("IK8TWX K1ABC -%1").arg(10 + i),
                                   -10, 1500 + (i + 1) * 25);
            ri.receivedAt = synthetic;
            eng.feedParsed({ri});
        }
        QCOMPARE(eng.atsSampleCount(), std::size_t(decodium::ft2::kAtsMaxSamples));
        QVERIFY(eng.atsHasConfidence());

        // Place clock 200ms before the predicted RX center
        // (= last_arrival + 1.5*slotPeriod = last + 5625ms).
        synthetic += std::chrono::milliseconds(5425);

        QSignalSpy atsSpy(&eng, &Ft2QsoEngine::atsOffsetApplied);
        // Force a real emit with a NEW msg via doubleClick TX1 (guard-bypass).
        eng.doubleClick("K1ABC", "FN42", 1500, -10, /*requestedTx=*/1);
        QVERIFY(std::holds_alternative<state::ReplyingTx1>(eng.state()));

        QVERIFY2(atsSpy.count() >= 1,
                 "atsOffsetApplied must fire on doubleClick-driven requestTx");
        int const off = atsSpy.last().at(0).toInt();
        QVERIFY2(off > 0 && off <= 500,
                 qPrintable(QStringLiteral("expected positive offset within ±500ms cap, got %1ms").arg(off)));
        QVERIFY2(off >= 150 && off <= 250,
                 qPrintable(QStringLiteral("expected ~200ms (clock placed 200ms before RX center), got %1ms").arg(off)));
        QCOMPARE(eng.lastAtsOffsetMs(), off);
    }

    // ----------------------------------------------------------------
    // ATS: a recommended delay larger than kAtsMaxCorrection (500ms)
    // must be clamped to the cap. Using clock = last_arrival yields a
    // raw recommendation of 5625ms → must be applied as 500ms.
    // ----------------------------------------------------------------
    void atsCapsAtMaxCorrection() {
        auto cfg = makeCfg();
        cfg.adaptiveTxSyncEnabled = true;
        Ft2QsoEngine eng(cfg, nullptr);
        TimePoint synthetic = Clock::now();
        ...
        for (int i = 0; i < 4; ++i) {
            synthetic += std::chrono::milliseconds(7500);
            DecodeRow ri = makeRow("K1ABC",
                                   QString("IK8TWX K1ABC -%1").arg(10 + i),
                                   -10, 1500 + (i + 1) * 25);
            ri.receivedAt = synthetic;
        ...
        // Trigger emit immediately (clock = last_arrival) — raw recommended
        // delay would be ~5625ms; engine must clamp to kAtsMaxCorrection.
        QSignalSpy atsSpy(&eng, &Ft2QsoEngine::atsOffsetApplied);
        eng.doubleClick("K1ABC", "FN42", 1500, -10, /*requestedTx=*/1);

        QVERIFY(atsSpy.count() >= 1);
        int const off = atsSpy.last().at(0).toInt();
        int const cap = static_cast<int>(std::chrono::duration_cast<std::chrono::milliseconds>(
                                             decodium::ft2::kAtsMaxCorrection).count());
        QCOMPARE(off, cap);
        QCOMPARE(eng.lastAtsOffsetMs(), cap);
    }
};
