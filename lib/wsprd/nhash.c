...
uint32_t nhash( const void *key, size_t length, uint32_t initval)
{
  ...
  if (HASH_LITTLE_ENDIAN && ((u.i & 0x3) == 0)) {
    ...
    /*
     * Do not use masked 32-bit loads for a partial word here.  They may look
     * harmless on ordinary page-aligned allocations, but still read past an
     * exact-size key (for example the 37-byte MSK144 shorthand hash field).
     * Byte reads preserve the little-endian hash value without relying on
     * padding beyond the caller-provided length.
     */
    k8 = (const uint8_t *)k;
    switch(length)
    {
