  ...
  while (true)
    {
      if (ir - l < kInsertionThreshold)
      ...
      else
        {
          ...
          while (true)
            {
              do { ++i; } while (values[indices[i - 1] - 1] < a);
              do { --j; } while (values[indices[j - 1] - 1] > a);
              if (j < i) break;
          ...
          if (jstack > kStackSize)
            return;
          if (ir - i + 1 >= j - l)
            { istack[jstack - 1] = ir; istack[jstack - 2] = i; ir = j - 1; }
          else
            { istack[jstack - 1] = j - 1; istack[jstack - 2] = l; l = i; }
        }
    }
}
