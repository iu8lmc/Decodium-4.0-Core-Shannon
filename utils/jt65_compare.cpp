...
void dump_stage (char const* label, std::array<int, 63> const& data)
{
  std::fprintf (stderr, "  %s:", label);
  for (int i = 0; i < 12; ++i)
    {
      std::fprintf (stderr, " %d", data[static_cast<size_t> (i)]);
    }
  std::fprintf (stderr, "\n");
}

void dump_pipeline_compare (decodium::legacy_jt::detail::PackedJtMessage const& packed)
{
  int dgen[13] {};
  int sent_raw[63] {};
  for (int i = 0; i < 12; ++i)
  ...
  std::array<int, 63> header_rs {};
  std::array<int, 63> wrapper_rs {};
  for (int i = 0; i < 63; ++i)
    {
      header_rs[static_cast<size_t> (i)] = sent_raw[i];
      wrapper_rs[static_cast<size_t> (i)] = sent_raw[i];
    }
  dump_stage ("rs", header_rs);

  std::array<int, 63> header_interleave = header_rs;
  std::array<int, 63> wrapper_interleave = wrapper_rs;
  decodium::legacy_jt::detail::interleave63 (header_interleave);
  decodium::legacy::interleave63_inplace (wrapper_interleave.data (), 1);
  dump_stage ("header interleave", header_interleave);
  dump_stage ("wrap   interleave", wrapper_interleave);

  std::array<int, 63> header_gray = header_interleave;
  std::array<int, 63> wrapper_gray = wrapper_interleave;
  for (int i = 0; i < 63; ++i)
    {
      header_gray[static_cast<size_t> (i)] =
          decodium::legacy_jt::detail::gray_encode (header_gray[static_cast<size_t> (i)]);
    }
  decodium::legacy::graycode65_inplace (wrapper_gray.data (), 63, 1);
  dump_stage ("header gray", header_gray);
  dump_stage ("wrap   gray", wrapper_gray);
}
...
bool compare_message (QString const& message, bool check_only)
{
  ...
  std::copy_n (input.constData (), 22, msg);
  msg[22] = '\0';

  gen65 (msg, &ichk, msgsent, tones, &itype);

  decodium::txmsg::EncodedMessage const encoded =
      decodium::txmsg::encodeJt65 (QString::fromLatin1 (msg, 22), check_only);
  QByteArray const cpp_input = decodium::legacy_jt::detail::fixed_ascii (
      QString::fromLatin1 (msg, 22).toLatin1 (), 22);
  ...
  decodium::legacy_jt::detail::CheckMessageResult const checked =
      decodium::legacy_jt::detail::chkmsg (trimmed);
  decodium::legacy_jt::detail::PackedJtMessage const packed = checked.nspecial == 0
      ? decodium::legacy_jt::detail::packmsg (checked.message)
      : decodium::legacy_jt::detail::PackedJtMessage {};
  if (!encoded.ok)
    {
      std::fprintf (stderr, "C++ encoder failed for '%s'\n", input.constData ());
      return false;
    }

  QByteArray const fortran_msgsent = fixed_22 (QByteArray (msgsent, 22));
  ...
  for (int i = 0; i < 126; ++i)
    {
      if (tones[i] != encoded.tones.at (i))
        {
          std::fprintf (stderr,
                        "tone mismatch for '%s' at %d: fortran=%d cxx=%d\n"
                        "  fortran state: itype=%d nc1=%d nc2=%d ng=%d k1=%d k2=%d\n"
                        "  cxx state    : itype=%d nc1=%d nc2=%d ng=%d\n",
                        input.constData (), i, tones[i], encoded.tones.at (i),
                        __packjt_MOD_jt_itype, __packjt_MOD_jt_nc1, __packjt_MOD_jt_nc2,
                        __packjt_MOD_jt_ng, __packjt_MOD_jt_k1, __packjt_MOD_jt_k2,
                        packed.itype, packed.nc1, packed.nc2, packed.ng);
          dump_pipeline_compare (packed);
          return false;
        }
    }
...
int main (int argc, char** argv)
{
  ...
  if (failures != 0)
    {
      std::fprintf (stderr, "JT65 compare failed: %d mismatches\n", failures);
      return 1;
    }
