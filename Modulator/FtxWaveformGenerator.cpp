...
{
  int const fox_nslots = clamp_int (foxcom_.nslots, 0, 5);
  int nmsg[4] {0, 0, 0, 0};
  int nbits = 0;
  int nb_mycall = 0;
  QString mycall;
  ...
  for (int slot = 0; slot < fox_nslots; ++slot)
    {
      ...
      for (int i = 1; i < words.size (); ++i)
        {
          ...
          int const next_index = std::min (i + 1, (int)(words.size () - 1));
...
void fox_bandlimit_inplace (QVector<float>& wave, int nslots, int nfreq, float width)
{
  ...
  int const copy_samples = std::min ((int)wave.size (), kFt8FoxNwave);
...
void ft2_fox_bandlimit_inplace (QVector<float>& wave, int nslots, int nfreq, float width)
{
  ...
  int const copy_samples = std::min ((int)wave.size (), kFt2FoxNwave);
  for (int i = 0; i < copy_samples; ++i)
    {
      time_domain[static_cast<size_t> (i)] = wave.at (i);
    }
  ...
  for (int slot = 0; slot < fox_nslots; ++slot)
    {
      ...
      QVector<float> const slot_wave = decodium::txwave::generateFt8Wave (encoded.tones.constData (),
                                                                          ...
                                                                          48000.0f,
                                                                          f0);
      int const samples = std::min ((int)wave.size (), (int)slot_wave.size ());
      for (int i = 0; i < samples; ++i)
        {
          wave[i] += slot_wave.at (i);
        }

      if (slot == fox_nslots - 1)
        {
                               ...
                               int* icmplx, int* nwave)
{
  ...
  QVector<float> const generated = decodium::txwave::generateFt8Wave (itone, symbol_count,
                                                                      ...
                                                                      *fsample, *f0);
  int const samples = std::min (output_samples, (int)generated.size ());
  for (int i = 0; i < samples; ++i)
    {
