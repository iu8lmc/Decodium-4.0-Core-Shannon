...
extern "C" void gen4_ (char* msg, int* ichk, char* msgsent, int itone[],
...
bool compare_message (QString const& message, bool check_only)
{
  ...
  std::array<int, 206> tones {};
  int itype = 0;
  int ichk = check_only ? 1 : 0;
  std::copy_n (input.constData (), 22, msg.data ());

  gen4_ (msg.data (), &ichk, msgsent.data (), tones.data (), &itype,
  ...
  decodium::txmsg::EncodedMessage const encoded =
      decodium::txmsg::encodeJt4 (QString::fromLatin1 (msg.data (), 22), check_only);
  QByteArray const normalized = decodium::legacy_jt::detail::fmtmsg (
      decodium::legacy_jt::detail::fixed_ascii (QString::fromLatin1 (msg.data (), 22).toLatin1 (), 22));
  decodium::legacy_jt::detail::PackedJtMessage const packed =
      decodium::legacy_jt::detail::packmsg (normalized);

  if (!encoded.ok)
    {
      std::fprintf (stderr, "C++ JT4 encoder failed for '%s'\n", input.constData ());
      return false;
    }
...
int main (int argc, char** argv)
{
  ...
  if (failures != 0)
    {
      std::fprintf (stderr, "JT4 compare failed: %d mismatches\n", failures);
      return 1;
    }
